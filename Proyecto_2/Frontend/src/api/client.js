   ...
   * Devuelve la lista de nombres de archivos disponibles para descargar.
   */
   ...
   * Descarga un archivo generado por /export directamente en el navegador.
   * Crea un <a> temporal con el blob y lo dispara automáticamente.
   *
   * En MOCK_MODE simula la descarga con un archivo de texto vacío.
   */
  export const descargarArchivo = async (nombre) => {
    console.log("Entró a descargarArchivo:", nombre);

    const res = await fetch(`${BASE}/export/download/${nombre}`);
    ...
    _dispararDescarga(blob, nombre);
  };

  /** Crea un <a> temporal y dispara la descarga del blob en el navegador. */
  function _dispararDescarga(blob, nombre) {
  ...
  /**
   * GET /
   * Consulta el estado del backend al arrancar la app.
   * Devuelve si hay modelos cargados en memoria desde persistence.py
   * y sus metadatos completos para reconstruir el estado del frontend.
   *
   * En MOCK_MODE simula que no hay modelos guardados (flujo normal).
   */
  export const checkEstado = async () => {
    if (MOCK_MODE) {
      return {
        modelos_en_memoria: { freelancers: false, resenas: false },
        meta_fl: null,
        meta_rv: null,
      };
    }
